...
(defn create-tree
    ...
    (if
        (= (count Q) 1)
        (first Q)
        (let [first (first Q) second (second Q) rest (drop 2 Q)]
            (create-tree (insert-into-queue (make-branch first second) rest) )
        )
    )
)
...
(defn find-huffman
    "Takes a Huffman tree T and a sequence if 1's and 0's. 
     Returns a map from symbols to Huffman codes.
     The algorithm as follows:
      (a) If T is a leaf node, return an empty map.
      (b) Otherwise, call find-huffman on the left child and the right child.
      (c) For each symbol in the map returned by find-huffman on the left child, 
          append a 0 to the code and store the result in the map.
      (d) For each symbol in the map returned by find-huffman on the right child, 
          append a 1 to the code and store the result in the map."

    [T binary-sequence]

    (if (isleaf? T)
        [[(T :value) binary-sequence]]
        (let [left-tree (T :left) right-tree (T :right)]
            (concat
                (find-huffman left-tree  (concat binary-sequence '(0)))
                (find-huffman right-tree (concat binary-sequence '(1)))
            )
        )
    )
...
(defn huffman-encode
    ...
    (let [
            tree (huffman-tree S) ;; The Huffman tree constructed from the provided sequence.
            symbol-to-code-map (huffman-codes tree) ;; The Huffman codes for each symbol in the tree, eg. {'a' => (1 0 1 1), 'b' => (0 1 0 1)}
            length (count S)  ;; The length of the provided sequence.
            bits (mapcat symbol-to-code-map S) ;; The sequence of bits encoding the sequence.
        ]
