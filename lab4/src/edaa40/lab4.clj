...
(defn- H'
    ...
    (if
        (empty? S)
        (concat P [a])
        (some       ;; some is a built-in function that takes a predicate and a list and returns the first element in the list that satisfies the predicate
            #(H'                ;; "#(..)" denotes an anonymous function. If :pre or :post isn't satisfied, the predicate is not satisfied.
                E               ;; the graph
                %               ;; the current node
                (disj S %)      ;; the set of nodes we haven't visited yet
                (concat P [a])  ;; the path we have traveled so far
            ) 
            (intersection S (image-of E a)) ;; the set of nodes we can visit next
            ;; We can visit the image of the current node a, but the nodes in this set must be in the
            ;; unvisited set S
        )
    )
...
(defn next-positions
    ...
    (intersection
        (set (map #(add-move pos %) Moves)) ;; the set of all moves from pos
        B                                   ;; the board
    )
...
(defn create-knights-move-relation
    ...
    (set
        (for [
                pos B                           ;; For each valid position pos on the board
                next-pos (next-positions pos B) ;; For each position next-pos that can be reached from pos
            ] 
            [pos next-pos]                      ;; return a relation with the pair (pos next-pos)
        )
    )
)
